...
> ## ⚠️ 状态更正（2026-09-19 核对，方案已分叉）
>
> | 方案 | 本文档状态 | **实际状态** |
> |---|---|---|
> | **方案 1 · VAE 剪枝头微调** | 「代码已就绪」+ 完整执行步骤 | ❌ **已废弃（v22，2026-09-07）**：微调后 decode 在测试集 **5/5 spconv int32 溢出**（`--max_coords 30000`；原版 VAE 同参数全部正常 → 溢出由微调补丁引起）。**下表 Step A / Step B 两个命令不要再执行** |
> | **方案 2 · LogitSharpener** | 「待实现」，文件清单标「新建」 | ⚠️ **仍未实现**（截至 2026-09-19 核对）：`logit_sharpener.py`、`train_logit_sharpener.py` 均**不存在**，`evaluate_3d_metrics.py` 也**没有** `--sharpener_ckpt` 参数。设计本身未变，**是唯一还挂着的「不动主模型」方案** |
>
> 另：本文档「背景」表中「空隙 ← `--max_coords` 截断丢 69%」的归因未变；但**治粗糙度的主嫌在 v24 已改为「降面」**（`dihedral 13.0°→38.0°`，GT 6.4°），粗糙并非只来自 16³ 结构。
>
> 主文档对应章节：`TRELLIS_LATO_完整实现流程_v2.md` 的 v20 §4（已同步标注）。
