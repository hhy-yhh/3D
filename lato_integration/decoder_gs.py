"""
Enhanced Gaussian Decoder with cross-attention to the original latent.

Inherits from TRELLIS's SLatGaussianDecoder and adds cross-attention
...
from trellis.models.structured_latent_vae.decoder_gs import (
    SLatGaussianDecoder as _SLatGaussianDecoder,
)
from trellis.models.structured_latent_vae.decoder_gs import (
    ElasticSLatGaussianDecoder as _ElasticSLatGaussianDecoder,
)
...
class EnhancedSLatGaussianDecoder(_SLatGaussianDecoder):
    """
    Enhanced SLat Gaussian Decoder with cross-attention to the original latent.

    The key enhancement over the original SLatGaussianDecoder is that after
    the main self-attention transformer body, a cross-attention block attends
    back to the original latent features. This follows LATO's architecture
    where each decoder level cross-attends to the latent for better feature
    propagation.

    Args:
        (same as SLatGaussianDecoder)
    ...
    def __init__(
        ...
        super().__init__(
            ...
            latent_channels=latent_channels + (model_channels if use_cross_attn else 0),
        ...
        if use_cross_attn:
            # Fix: we passed inflated in_channels to super, but base class stores it.
            # We need to override the input_layer to accept just latent_channels.
    ...
    def forward(self, x: sp.SparseTensor, original_latent: Optional[sp.SparseTensor] = None) -> List[Gaussian]:
        """
        ...
        """
        h = super(_SLatGaussianDecoder, self).forward(x)  # SparseTransformerBase.forward
...
class EnhancedElasticSLatGaussianDecoder(SparseTransformerElasticMixin, EnhancedSLatGaussianDecoder):
    """
    Enhanced SLat Gaussian Decoder with elastic memory management and cross-attention.
    """
    pass
