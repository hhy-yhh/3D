...
**当前版本：v20 (2026-08-27)** — 2026-08-28 复核修正见文末 v21
...
> ⚠️ **2026-08-28 复核：「GT ~5000（0.24%）」「th>2 接近真实数量」数字错误。** GT occupancy（`ss_occupancy_128_v2`）实测均值 **~38,577（1.8%）**；SS th=2.0 = 97,969（**2.5×** GT，非 20×）。厚壳真实但程度更轻，见文末 v21。
...
> ⚠️ **2026-08-28 复核：本节第 1 点（fp16 不需要 / 全部 16384）结论错误。** 真实训练数据 `lato_latents_v2` 的 coords 为 26K-68K（均 ~41K，含 52416）；「全部 16384」是损坏本地副本（Z=127 扁平 → 128²）。fp16 / 数量问题重新打开，见文末 v21。
...
> ✅ **2026-08-28 复核确认：本节结论正确。** 旧度量在真实 pred+test GT（毫米）上精确复现 **0.2700**，新度量 0.0022；grid 模式 CD 0.0022 / HD 0.105 / NC 0.46 全部复现。详见文末 v21。
...
## v20 后续实验与结论 (2026-08-27)

### 1. 观感问题确认（v19 之后）

v19 修复了 CD 度量 + 三角汤，几何正确（CD 0.002）。但**观感仍不理想**，网格表示逐层尝试：

| 手段 | 结果 |
|---|---|
| grid 四边形化 | 方块感（512³ 粒度）、**非流形** → Laplacian 平滑炸尖刺（撤销） |
| Poisson 重建 | 从「方块」变「纸糊」（PCA 法线带 512³ 阶梯偏差、面太碎） |
| Poisson + 平滑 | 更光滑；但 **Hausdorff 0.10** → 有一处局部特征差 0.10（薄孔/翅片被磨掉） |
| 生成路径（SS）最终 | CD 0.002 形状对、NC 0.66，但表面糙/坑洼/缝隙 |

### 2. 服务器 latent 验证（关键澄清）

- **本地副本的 GT latent 是退化的**（coords 全部 Z=127，一个平面）——本地副本损坏，不具代表性。
- **服务器上的 GT latent 是正常 3D**（Z 有 46~54 个不同值）→ **SLat 训练数据正常**。
- 结论：担心的「扁平 latent 污染 SLat 训练」**不成立**，撤销该假设。

### 3. A 组（GT 输入）诊断

- A 组用 GT latent + GT feats（完美输入），却只重建出「半截模型」——可能来自 Poisson 密度裁剪（`crop_density_quantile=0.1`）或解码行为。
- 阈值 0.2→0.0：L2 从 162K→532K（更密），但仍半截。
- **注意**：A 组 latent 与 CD GT mesh 朝向差 ~90°（Y/X 长短轴互换），A 组 CD 数字**不能当有效基线**。
...
|---|---|
| CLIP | ✅ ViT-L/14，一致 |
| SS / SLat Flow | ✅ `EnhancedXxx` 继承官方类，训练/推理同一 config 构建 |
| VoxelVAE | ⚠️ 差 `use_checkpoint=True`（官方 False）——推理无影响，无害 |
| ConnectionHead | ✅ channels=1024 = 官方 512×2 |
| **LatoStructureHead** | ⚠️ **唯一真偏差**：自定义模块（替代 TRELLIS `SparseStructureDecoder`），16³→128³ 糊 occupancy（Bug 26） |

**结论：没有「模块不一致导致的连锁问题」**。表面粗糙来自：① 结构生成粗（16³）；② latent 误差（MSE 0.2）；③ 重建层选择（Poisson 裁剪/平滑）。

### 6. 最终结论

- 生成几何正确（CD 0.002）。
- 观感限制 = 结构粗（16³）+ latent 误差（MSE 0.2）+ 重建选择。
- 不加数据的前提下，可行的提升方向：**VAE 剪枝头微调** / **LogitSharpener**。
- 治本（需资源）：新数据 / 32³ SS encoder。

---

## v21 复核验证与修正 (2026-08-28)

> 逐项复核 v18/v19/v20 关键结论（服务器实测），修正 3 处错误。

1. **✅ v19 确认正确**：test GT 毫米（`/3D/database/`，diag 432.5）、pred 归一化（diag~1.1）；旧度量 **0.2700**（精确复现文档「恒 ~0.27」）、新度量 0.0022；grid CD 0.0022 / HD 0.105 / NC 0.46 全部复现。「生成几何正确」成立，v19 的 CD 度量 bug 是真实主因。
2. **❌ v18 第 1 点（fp16 不需要 / 全部 16384）错误**：服务器 `lato_latents/` 不存在；真实 `lato_latents_v2` coords 26K-68K（均 ~41K，含 52416）。「全部 16384」是损坏本地副本（Z=127 扁平 → 128²）。fp16 / 数量问题**重新打开**；实验 A 的「数量不是瓶颈」推论撤回。
3. **⚠️ Bug 26「GT ~5000」错误**：GT occupancy 均值 ~38,577（1.8%）；SS th=2.0 = 97,969（2.5×）。厚壳真实但更轻。推理需 `--max_coords 16384` 防 OOM。

**路径提醒**：`lato_latents`（v1，损坏副本）≠ `lato_latents_v2`（v2，训练/评估真实数据）。核对 latent 一律看 v2。

4. **数量瓶颈对照实验定论**（`--max_coords` 扫描，同一样本 20250423_1800_838505）：
...
## v26 问题定位总结 (2026-09-19)

> 回答「点云生成看着很好，为什么最终 mesh 会这样」。

### 症状 → 根因

| 症状 | 实测数据 | 根因 | 与 SLat Flow 误差 |
|---|---|---|---|
...
```
点云 = 0 厚度的采样（399,308 点，只占 bbox 的 1.59%）
     → 只描述「表面在哪」，不含「内部在哪」

建面唯一可行的内外判断机制 = 拟合隐式函数（Poisson）
     → 「跨过点云的空隙」和「跨过镂空」在几何上是同一个动作
     → 必然填死镂空
```

不是 Poisson 有 bug，是**两个要求互相矛盾**：跨过点云空隙（否则表面是筛子） vs 不跨过镂空（否则特征丢失）。

### 已穷尽的重建方式

| 方式 | 结果 |
|---|---|
| Poisson | 表面最好（13.0°），**必填镂空** |
| voxel | 海绵（点云是散点非实心，NC 0.4478） |
| grid / MC | 碎成 29.5 万块 / 塌 |
| **ball** | **碎成 32,708 块**（`dihedral p50=0.0 p90=90.0`，三角形全贴格点平面） |
| alpha | open3d 在 40 万点上崩（`invalid tetra`） |

### 补洞：可用，但收益有上限

- **配置**：`--repair --max_loop 250`。`--max_loop` **必须 < 2025** —— 已实测确认 #0（2025 顶点）就是镂空开口，补上就没了
- **实测**：`holes 261→2`、`boundary_edges 6,646→2,743`（剩下的全是镂空）、`v` 不变（50,523）
- **上限**：补的 3,361 个面摊在 5 万顶点上，**肉眼看不见** —— 这是这条路的天花板，不是没做对
- **不治粗糙**：`dihedral` 补前 38.0° → 补后 37.9°，纹丝不动

### 待定：SLat Flow MSE 0.17~0.22 的影响

- v21 已定论：MSE 0.2 是**对数收敛下限**，不是没训好。根因是 **prompt 只描述 ~15 个参数、latent 有几万自由度 → 映射欠定**，再训也降不了
- **嫌疑**：它导致 decode 出的点云是「厚糊团」而非干净薄壳
- **未验证**：需一次干净对照（GT latent 走同一条 decode → 比层厚 / 重合点比例 / 连通块数 / 稀疏率）
