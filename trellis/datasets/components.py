...
class StandardDatasetBase(Dataset):
    ...
    def __init__(self,
        ...
        for root in self.roots:
            ...
            self._stats[key] = {}
            metadata = pd.read_csv(os.path.join(root, 'metadata.csv'))#读取metadata.csv文件
            self._stats[key]['Total'] = len(metadata)
            metadata, stats = self.filter_metadata(metadata)
            self._stats[key].update(stats)
            self.instances.extend([(root, sha256) for sha256 in metadata['sha256'].values])#将符合条件的实例添加到instances列表中
...
class TextConditionedMixin:#提供文本条件加载功能 (从 metadata.csv 读取 captions)
...
class ImageConditionedMixin:#提供图像条件加载功能 (从 renders_cond/ 读取渲染图)
