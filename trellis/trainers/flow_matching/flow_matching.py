...
#定义 通用 Flow Matching 训练器，处理稠密张量数据
class FlowMatchingTrainer(BasicTrainer):
    ...
    #前向扩散过程--执行加噪操作，从干净数据 x_0 生成带噪数据 x_t
    ...
    def training_losses(
        ...
        **kwargs
        ...
        pred = self.training_models['denoiser'](x_t, t * 1000, cond, **kwargs)#将 x_t 和时间步 t 输入 denoiser 网络预测速度场。
        assert pred.shape == noise.shape == x_0.shape
        target = self.get_v(x_0, noise, t)#调用 get_v 计算真实速度场作为目标。
        terms = edict()
        terms["mse"] = F.mse_loss(pred, target)
        terms["loss"] = terms["mse"]

        # log loss with time bins--将时间步划分为 10 个区间，并计算每个区间的平均 MSE 损失。
    ...
    def run_snapshot(
        ...
        sample = []
        cond_vis = []
        #从数据集中采样 num_samples 个样本，分批处理，每批大小为 batch_size。
        #对于每个批次，获取干净数据 x_0 和条件 cond，然后生成带噪数据 x_t，并使用 denoiser 模型预测速度场。最后将预测结果与真实速度场进行比较，计算损失。
        for i in range(0, num_samples, batch_size):
            batch = min(batch_size, num_samples - i)
            data = next(iter(dataloader))
            data = {k: v[:batch].cuda() if isinstance(v, torch.Tensor) else v[:batch] for k, v in data.items()}
            noise = torch.randn_like(data['x_0']) # 生成完整噪声
            sample_gt.append(data['x_0'])# 标准拼接
        ...
        #获取真实样本和生成样本将它们拼接成一个字典返回，同时也返回条件可视化结果。
