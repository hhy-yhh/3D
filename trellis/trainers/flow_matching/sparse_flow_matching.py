...
#这个文件定义了稀疏流匹配训练器类SparseFlowMatchingTrainer，
#以及其子类SparseFlowMatchingCFGTrainer、TextConditionedSparseFlowMatchingCFGTrainer和ImageConditionedSparseFlowMatchingCFGTrainer，
#分别用于稀疏扩散模型的训练、带有分类器自由引导的训练、文本条件训练和图像条件训练。
...
class SparseFlowMatchingTrainer(FlowMatchingTrainer):
    ...
    def training_losses(
        ...
        **kwargs
        ...
        """
        noise = x_0.replace(torch.randn_like(x_0.feats))#替换特征，保留结构
        ...
        terms = edict()
        terms["mse"] = F.mse_loss(pred.feats, target.feats)# 只计算特征部分的MSE
    ...
    def run_snapshot(
        ...
        for i in range(0, num_samples, batch_size):
            ...
            noise = data['x_0'].replace(torch.randn_like(data['x_0'].feats)) # 保持稀疏结构生成噪声
            sample_gt.append(data['x_0'])# 稀疏张量专用拼接
