...
#这个文件定义了一个基础训练器类 Trainer，提供了训练循环、数据加载、模型保存和日志记录等功能。它是一个抽象类，需要在子类中实现具体的模型训练逻辑。
class Trainer:
    """
    ...
    #运行训练循环，直到达到最大步数。它会加载数据、执行训练步骤、记录日志、保存模型检查点，并在指定的间隔内进行采样和检查 DDP 状态。
    def run(self):
        """
        Run training.
        """
        ...
        while self.step < self.max_steps:
            ...
            if self.is_master:
                ...
                if self.step % self.i_log == 0:
                    ## save to log file
                    log_str = '\n'.join([
                        f'{step}: {json.dumps(log, default=str)}' for step, log in log
                    ])
